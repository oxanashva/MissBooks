export function RateByStars({ name, value, handleInput, readOnly = false, size = 20, totalStars = 5 }) {
    const starSVG = (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            width={size}
            height={size}
            viewBox="0 0 36 36">
            <path fill="currentColor" d="M27.287 34.627c-.404 0-.806-.124-1.152-.371L18 28.422l-8.135 5.834a1.97 1.97 0 0 1-2.312-.008a1.971 1.971 0 0 1-.721-2.194l3.034-9.792l-8.062-5.681a1.98 1.98 0 0 1-.708-2.203a1.978 1.978 0 0 1 1.866-1.363L12.947 13l3.179-9.549a1.976 1.976 0 0 1 3.749 0L23 13l10.036.015a1.975 1.975 0 0 1 1.159 3.566l-8.062 5.681l3.034 9.792a1.97 1.97 0 0 1-.72 2.194a1.957 1.957 0 0 1-1.16.379z" />
        </svg>
    )

    const starsArray = Array.from({ length: totalStars }, (_, i) => i + 1);

    if (readOnly) {
        return (
            <div className="rate-by-stars">
                {starsArray.map(num => (
                    <span key={num} className={num <= value ? "selected" : ""}>
                        {starSVG}
                    </span>
                ))}
            </div>
        )
    }

    return (
        <div className="rate-by-stars">
            {starsArray.map(num => (
                <label key={num} htmlFor={num}>
                    <input
                        type="radio"
                        id={num}
                        name={name}
                        value={num}
                        checked={value === num}
                        onChange={handleInput}
                        className="sr-only"
                    />
                    <span className={num <= value ? "selected" : ""}>
                        {starSVG}
                    </span>
                </label>
            ))}
        </div>
    )
}

// https://mui.com/material-ui/react-rating/
// https://medium.com/@rj03012002/creating-a-star-rating-component-in-react-a-css-deep-dive-d376e6d99ea9

// and more search: "stars widget react radio button"